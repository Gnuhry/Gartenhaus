...
namespace Gartenhaus
{
    ...
    public class Arduino : DatabaseCommunication
    {
        ...
        private static void Client1()
        {
            if (Sendstring == 0)
            {
                Client.Arduino_Send(Sendint, Sendint+ "§-1,-1:-1%-1;-1_-1+-1*-1");
                return;
            }
            string[] PlantData = Plant.GetAll(Sendstring);
            string send=Sendint+ "§" + PlantData[1]+ "," + PlantData[2]+":" + PlantData[3]
            +"%" + PlantData[4]+";" + PlantData[5]+"_" + PlantData[6]+ "+" + PlantData[7]+"*" + PlantData[8];
