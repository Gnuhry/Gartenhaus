    ...
    private void SetGraph(int mode, int splitter) {
        ...
        for (String aData : data) {
            if (!(aData.toCharArray()[0]==':')) {
                ...
                try {
                    Log.e("graph_hung",data[splitter].split("-")[0].substring(1)+"=="+aData.split("_")[0]+"_"+(data[splitter].split("-")[0].substring(1).equals(aData.split("_")[0])));
                    if(data[splitter].split("-")[0].substring(1).equals(aData.split("_")[0])) {
                        Log.e("graph_hung","ka");
                        if (data[splitter].split("_").length > 1) {
                            max.appendData(new DataPoint(format.parse(aData.split("_")[2]), Float.parseFloat(data[splitter].split("_")[mode + 4])), false, counter);
                            if (mode != 3) {
                                min.appendData(new DataPoint(format.parse(aData.split("_")[2]), Float.parseFloat(data[splitter].split("_")[mode + 1])), false, counter);
                            }
                        }
                        Log.e("graph_hung",aData.split("_")[2]+"__"+aData.split("_")[mode + 3].replace(",", "."));
