            ...
            if (bytesRead > 0)
            {
                ...
                if (content.IndexOf("<EOF>") > -1)
                {
                    ...
                    switch (txt.ToLower())
                    {
                        ...
                        case "set data": reponse = SQLCommunication.SetData(Convert.ToSingle(help[2]), Convert.ToSingle(help[3]), Convert.ToSingle(help[4]), Convert.ToSingle(help[5]),DateTime.Now.ToString("hh:mm:ss dd.MM.YYYY"), Convert.ToInt32(help[1])); break; //set data [ID:int] [Temperatur:float] [Feuchtigkeit:float] [Humid:float] [UV:float]
